...
parser.add_argument(
    "--camera", type=int, default=0, help="Índice de la cámara (0=built-in, 1=externa)"
)
...
model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "emotion.onnx")
...
# SCRFD — detección de caras
detector = FaceDetector(
    det_size=(FRAME_W, FRAME_H),
    conf_threshold=0.3,  # permissive; CONF_THRESHOLD filters in the loop
)
...
out = cv2.VideoWriter(
    OUTPUT_PATH, cv2.VideoWriter_fourcc(*"mp4v"), 15, (FRAME_W, FRAME_H)
)
...
try:
    while True:
        ...
        if frame_count % 30 == 0:
            ...
            print(
                f"[Det] frame={frame_count} raw={n_raw} passed={face_count} "
